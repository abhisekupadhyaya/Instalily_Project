...
export class QueryService {
    ...
    async processUserQuery(chatId: string, chatQuery: string) {
        try {
            const queryDocument = await this.getOrCreateQueryDocument(chatId);
            ...
            const pageUrl = queryDocument.pageUrl || 'none';
            
            return { success: true, pageUrl };
        } catch (error) {
            console.error('Error processing user query:', error);
            return { success: false, message: 'An error occurred while processing the user query', error: error.message };
        }
    }
    
    async processIntelligenceResponse(chatId: string, ollamaResponse: any) {
        try {
            const queryDocument = await this.getOrCreateQueryDocument(chatId);
            ...
            console.error('Error processing intelligence response:', error);
            return { 
                success: false, 
                message: 'An error occurred while processing the intelligence response', 
                error: error.message 
            };
        }
    }
